...
#define JMP_IF(con, err, jmp, str, ...) do { if (con) { g_error = err; fprintf(stderr, str, ##__VA_ARGS__); goto jmp; } } while(0);

#define RET_IF(con, err, ret, str, ...) do { if (con) {  g_error = err; fprintf(stderr, str, ##__VA_ARGS__); return ret; } } while(0);
